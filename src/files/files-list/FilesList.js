...
export const FilesList = ({
  ...
  onShare, onSetPinning, onInspect, onDownload, onRemove, onRename, onNavigate, onAddFiles, onMove, handleContextMenuClick, t
}) => {
  ...
  const emptyRowsRenderer = () => (
    <>
      ...
      <div style={{ margin: '80px auto 5px', width: 'fit-content' }}>
        <DangerIcon />
      </div>
      <div className='pv3 b--white-60 tc white w95fa fs14 spacegrotesk'>
        No files here yet!
      </div>
      <p className='fs12 spacegrotesk' style={{ color: 'gray', textAlign: 'center' }}>
        Add files to your local IPFS node by <br /> clicking the Import button above.
      </p>
  ...
  return (
    <section ref={drop} className={classnames('FilesList no-select sans-serif border-box w-100 flex flex-column', className)}>
      {showLoadingAnimation
        ? <LoadingAnimation />
        : <Fragment>
          <header className='w95fa pv3 flex items-center flex-none' style={{ paddingRight: '1px', paddingLeft: '5px', borderBottom: '1px solid #312F62' }}>
            <div className={checkBoxCls}>
              <Checkbox checked={allSelected} onChange={toggleAll} aria-label={t('selectAllEntries')} />
            </div>
            <div style={{ color: 'rgba(255, 255, 255, 1)' }} className='f6 flex-auto spacegrotesk'>
