  ...
  return (
    <section className='bb b--black-10'>
      <div className='flex items-center'>
        <Checkbox className='pv3 pl3 pr1 flex-none' onChange={onChange} checked={enabled} label={
          <span className='fw5 f6 w95fa'>{label}</span>
        } />
        <div className='truncate fw4 f6 flex-auto'>&ndash; {summary}</div>
  ...
  return (
    <React.Fragment>
      <Checkbox className='dib' onChange={doToggleAnalytics} checked={analyticsEnabled} label={
        <span className='w95fa f6 white'>
          {t('AnalyticsToggle.label')}
        </span>
      } />
      <div className='w95fa f6 white lh-copy'>
        <p>{t('AnalyticsToggle.paragraph1')}</p>
        <RetroButton active={isOpen} width='200px' onClick={() => setIsOpen(!isOpen)}>
          <RetroText>
